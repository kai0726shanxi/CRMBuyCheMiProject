...
import com.buychemi.crm.net.exception.ApiException
...
fun Context.getTime4String(time: String): String {
    ...
    //获取想要的日期格式(输出的日期格式)
    var outputsdf: SimpleDateFormat = SimpleDateFormat("yyyy-MM-dd HH:mm")
...
fun Context.getTime(time: String): String {
    ...
    var inputsdf: SimpleDateFormat = SimpleDateFormat("yyyy-MM-dd HH:mm:ss")
    //获取想要的日期格式(输出的日期格式)
    var outputsdf: SimpleDateFormat = SimpleDateFormat("MM-dd HH:mm")
    var date: Date = inputsdf.parse(time)
    return outputsdf.format(date)
}
