package com.buychemi.crm.utils;
...
public class SpUtil {
     ...
     * @param context 上下文
     * @param key     字符串的键
     * @param datalist   list值
     */
    public static  <T> void setDataList(Context context, String key, ArrayList<T> datalist) {
        SharedPreferences preferences = getSp(context);
        //存入数据
        Editor editor = preferences.edit();
    ...
    public static <T> ArrayList<T> getDataList(Context context, String tag) {
        ...
        String strJson = SpUtil.getString(context,tag, null);
        if (null == strJson) {
            return datalist;
        }
        Gson gson = new Gson();
