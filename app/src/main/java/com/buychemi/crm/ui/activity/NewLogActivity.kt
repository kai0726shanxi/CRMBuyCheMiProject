...
import com.buychemi.crm.mvp.presenter.NewLogPresenter
 ...
 */
class NewLogActivity : BaseActivity(), NewLogContract.View, View.OnClickListener {
    ...
    private val mPresenter: NewLogPresenter by lazy { NewLogPresenter() }
    ...
    private fun pushdata(data: ArrayList<UpImagesEntity>?) {
        ...
        map["remarkDate"] = tv_time.text.toString()
        map["description"] = et_content.text.toString()
        map["checkInLocation"] = et_address.text.toString()
        map["markValue"] = mCheck
        if (data != null && data.size > 0) {
            ...
            map["image"]=strbuf.toString()
        }
        mPresenter.getNewLogdata(map)
    }
    ...
    override fun onNewLogdata(data: String?) {
        showToast("创建成功")
        finish()
    }
